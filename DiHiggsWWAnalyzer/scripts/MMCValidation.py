...
#	hist3.SetMarkerColor(ROOT.kGreen)
   ...
   #     hist4.SetFillStyle(4050)
    ...
    c1.SaveAs("Dihiggs_MMC_style1_%s"%pic_name+"_B6.pdf")
    c1.SaveAs("Dihiggs_MMC_style1_%s"%pic_name+"_B6.png")
    c1.SaveAs("Dihiggs_MMC_style1_%s"%pic_name+"_B6.C")
...
#    todraw = "%s"%yaxis+":%s"%xaxis
 ...
 #   b1.Draw()
    ...
    c1.SaveAs("Dihiggs_MMC_%s"%pic_name+"_B6.pdf")
    c1.SaveAs("Dihiggs_MMC_%s"%pic_name+"_B6.png")
...
def drawh2Mass_combined(file,x_bins,pic_name):
    ...
    #testhist = hist_1D(test, h2tohhmass, x_bins, cut,0)
    #testhist.Draw()
    #c1.SaveAs("test.png")
    hslist = []
    while sub_key: 
	...
	if n%9 == 8:
		c1.cd()
        	tex2.Draw("same")
		c1.SaveAs("Dihiggs_MMC_%s"%pic_name+"_B6_%d"%(n/9)+".pdf")
    		c1.SaveAs("Dihiggs_MMC_%s"%pic_name+"_B6_%d"%(n/9)+".png")
    		c1.SaveAs("Dihiggs_MMC_%s"%pic_name+"_B6_%d"%(n/9)+".C")
        n = n+1
...
def drawoffshellWmass_combined(file,x_bins,pic_name):
	...
	tree = sub_key.ReadObj()
        #tree.Print()
        hs = ROOT.THStack("hs_%d"%(n%9)," ");
        hist1 = hist_1D(tree, offshellWmass, x_bins, cut,0)
        hist1.SetFillColor(ROOT.kRed)
        #hist1.Draw("E3") 
	hs.Add(hist1)
        
        hist2 = hist_1D(tree, offshellWmass, x_bins, cut,1)
        hist2.SetLineColor(ROOT.kBlue)
        hist2.SetLineWidth(4)
        #hist2.Draw("sameE3") 
	hs.Add(hist2)
   
        hist3 = hist_1D(tree, offshellWmass, x_bins, cut,2)
        hist3.SetLineColor(ROOT.kGreen)
        hist3.SetLineWidth(4)
        #hist3.Draw("sameE3") 
	hs.Add(hist3)

        hist4 = hist_1D(tree, offshellWtruemass, x_bins, cut,0)
        hist4.SetFillColor(ROOT.kMagenta)
        max = hist1.GetMaximum()
        bin_max = hist4.GetMaximumBin()
	hist4.SetBinContent(bin_max, max/4.0)
...
def monitoringMMC(file):
	...
	hs10 = gethiststack(tree, todraw10, todrawtrue10, eta_bins, hs_title10)
	
	#phi of nuonshellW	
        todraw11 = "nu_onshellW_phi"
	todrawtrue11 = "phi_nuonshellW_true"
        hs_title11 = "#phi_{#nu}^{onshellW}"
	hs11 = gethiststack(tree, todraw11, todrawtrue11, phi_bins, hs_title11)
...
#	hist4_2.SetTitleOffset(0.6)
	...
	hs12 = gethiststack(tree, todraw12, todrawtrue12, eta_bins, hs_title12)
	
	#phi of nuonshellW	
        todraw13 = "nu_offshellW_phi"
	todrawtrue13 = "phi_nuoffshellW_true"
        hs_title13 = "#phi_{#nu}^{offshellW}"
	hs13 = gethiststack(tree, todraw13, todrawtrue13, phi_bins, hs_title13)
	...
	c1.Update()
    	...
    	tex = ROOT.TLatex(0.15,.954,"Performance of MMC,#slash{E}_{T}=%d"%met+", %s"%(tree.GetTitle()))
    	tex.SetTextSize(0.04)
    	tex.SetTextFont(42)
    	tex.SetNDC()
	tex.Draw("same")
	c1.Update()
        c1.SaveAs("%s"%(tree.GetTitle())+"_0401_test_B6.pdf")
        c1.SaveAs("%s"%(tree.GetTitle())+"_0401_test_B6.png")
        c1.SaveAs("%s"%(tree.GetTitle())+"_0401_test_B6.C")
#	if m>1:
#		break
	m = m+1
...
#cut_weight should either "weight && (control<2)" or "weight" or "weight && (control>1)"
    ...
    xminBin = float(x_bins[1:-1].split(',')[1])
    xmaxBin = float(x_bins[1:-1].split(',')[2])
    name = "hist1_%s"%tree.GetTitle()+"_%s"%todraw+"_%d"%i
...
#cut_weight should either "weight && (control<2)" or "weight" or "weight && (control>1)"
    ...
    yminBin = float(y_bins[1:-1].split(',')[1])
    ymaxBin = float(y_bins[1:-1].split(',')[2])
    name = "hist2_%s"%tree.GetTitle()+"_%s"%todraw+"%d"%i
...
if __name__ == "__main__":
    ...
    #file = "/fdata/hepx/store/user/taohuang/Hhh/DiHiggs-1M-B6-1071409.root"
    file = "/fdata/hepx/store/user/taohuang/Hhh/DiHiggs_100k_0401_wmass_B6.root"
  ...
  #  draw1D(file, dir, h2tohhmass, title1, h2massbins, h2mass, tex1, pic1)
    ...
    pic2 = "evt843_h2Mass_incorrect_0401_test"
   # draw1D(file, dir, h2tohhmass, title1, h2massbins_2, h2mass, cut,0, tex2, pic2)
    ...
    pic3 = "evt843_h2Mass_both_0401_test"
  #  draw1D(file, dir, h2tohhmass, title1, h2massbins_3, h2mass, cut,2, tex3, pic3)
    #draw2D(file,dir,"nu_onshellW_phi:nu_onshellW_eta","eta_nuonshellW_true","phi_nuonshellW_true","#phi Vs #eta of nu_onshell","(50,-6,6)","#eta","(30,-3.14,3.14)","#phi","tex","etaVsphi_nuonshellW_0401_test")
    #draw2D(file,dir,"nu_offshellW_phi:nu_offshellW_eta","eta_nuoffshellW_true","phi_nuoffshellW_true","#phi Vs #eta of nu_offshell","(50,-6,6)","#eta","(30,-3.14,3.14)","#phi","tex","etaVsphi_nuoffshellW_0401_test")
    """ 
    offshellWmass = "offshellW_Mass"
    titleW1 = "off-shell W mass from MMC, Event843 "

    offshellWmassbins = "(50,0,50)"
    pic_offshellW = "offshellW_combined_0401_test"
    ...
    nuetabins= "(100,-7,7)"
    pic_offshellnu_eta = "offshellW_nueta_0401_test"
    ...
    pic_onshellnu_eta = "onshellW_nueta_0401_test"
    ...
    nuptbins = "(60,0,120)"
    pic_offshellnu_pt = "offshellW_nupt_0401_test"
    ...
    nuptbins = "(60,0,120)"
    pic_onshellnu_pt = "onshellW_nupt_0401_test"
    ...
    picW1 = "evt843_offshellWmass_correct_0401_test"
  #  draw1D(file, dir, offshellWmass, titleW1, offshellWmassbins, offshellWmass_x, cutW1, texW1, picW1)
    ...
    picW2 = "evt843_offshellWmass_incorrect_0401_test"
  #  draw1D(file, dir, offshellWmass, titleW1, offshellWmassbins, offshellWmass_x, cutW2, texW2, picW2)
    ...
    picW3 = "evt843_offshellWmass_both_0401_test"
