...
def monitoringMMC(file,cut = ["weight","weight*(control<2)","weight*(control>1)"]):
   ...
   # cut_weight1 = ["weight1","weight1*(control<2)","weight1*(control>1)"]
	...
	hist1_1.SetLineColor(ROOT.kRed)
        ...
        hist1max = hist1_2.GetBinContent(hist1_2.GetMaximumBin())
        array_contour1 = array('d',[hist1max])
        i = 0.8
        while i > 0.0:
		array_contour1.append(hist1max*i) 
	 	i = i-0.1
        hist1_1.SetContour(len(array_contour1), array_contour1) 
	...
	graph1_muon.SetMarkerSize(1)
        ...
        hist1_1.GetXaxis().SetTitle("#eta_{#nu}^{onshellW}")
        hist1_1.GetXaxis().SetTitleSize(0.08)
        hist1_1.GetXaxis().SetTitleOffset(0.6)
        hist1_1.GetYaxis().SetTitle("#phi_{#nu}^{onshellW}")
        hist1_1.GetYaxis().SetTitleSize(0.08)
        hist1_1.GetYaxis().SetTitleOffset(0.6)
        hist1_1.SetTitle(" ")
	...
	hist4_1.SetLineColor(ROOT.kRed)
        ...
        hist4max = hist4_2.GetBinContent(hist4_2.GetMaximumBin())
        array_contour4 = array('d',[hist4max])
        i = 0.8
        while i > 0.0:
		array_contour4.append(hist4max*i) 
	 	i = i-0.1
        hist4_1.SetContour(len(array_contour4), array_contour4) 
	...
	graph4_muon.SetMarkerSize(1)
        ...
        hist4_1.GetXaxis().SetTitle("#eta_{#nu}^{offshellW}")
        hist4_1.GetXaxis().SetTitleSize(0.08)
        hist4_1.GetXaxis().SetTitleOffset(0.6)
        hist4_1.GetYaxis().SetTitle("#phi_{#nu}^{offshellW}")
        hist4_1.GetYaxis().SetTitleSize(0.08)
        hist4_1.GetYaxis().SetTitleOffset(0.6)
        hist4_1.SetTitle("  ")
...
#	hist1_3.Draw("CONT3same")
	...
	c1.Update()
		...
		tex.Draw("same")
	c1.Update()
        c1.SaveAs("ttbar%s"%(tree.GetTitle())+"_0420_weight_B3.pdf")
        c1.SaveAs("ttbar%s"%(tree.GetTitle())+"_0420_weight_B3.png")
        c1.SaveAs("ttbar%s"%(tree.GetTitle())+"_0420_weight_B3.C")
        ##########################################
        ##### fill hist_h2
...
def drawh2Mass_combined(file, cut="weight"):
    ...
    hist_h2 = ROOT.TH1F("hist_h2"," ",200,200,800)
...
#		break;    
    ...
    hist_h2.SetTitle("ttbar sample M_{H} reconstruction from MMC")
   ...
   # hist_h2true.Draw("same")
    legend.Draw("same")
    h2Mass_c.SaveAs("ttbarMMC_h2Mass_0414_weight_1M_B3.pdf") 
    h2Mass_c.SaveAs("ttbarMMC_h2Mass_0414_weight_1M_B3.png") 
    h2Mass_c.SaveAs("ttbarMMC_h2Mass_0414_weight_1M_B3.C") 

#___________________________________________________________________________
def geth2MassMostProba(t, cut = "weight"):

    name = t.GetTitle()
    b1 = ROOT.TH1F("%s"%name,"b1",1000,100,1100)
...
if __name__ == "__main__":
    ...
    #file = "/fdata/hepx/store/user/taohuang/Hhh/DiHiggs_100k_0420_newframe_B3.root"
    file = "/eos/uscms/store/user/tahuang/DiHiggs/ttbar_10k_0416_B3.root"
